export const NodeWeightValue = 20;
...
export class Grid {
  ...
  buildGrid(): void {
    ...
    this.targetNode.weight = this.calculateAbsoluteNodeWeight(
  ...
  removeWall(row: number, column: number): void {
    ...
    const indexToRemove = this.walls.findIndex(
      (wall) => wall.rowIndex === row && wall.columnIndex === column
    );
    if (indexToRemove !== -1) {
      // Remove from walls
    ...
    // Update nodes
    this.updateNode(
      this.getNode(row, column),
      new Node(row, column, Infinity, NodeType.Normal)
    );
    // Update the weights
    this.updateNodesWeight();
  }

  addWeight(row: number, column: number): void {
    // Initialize the new node
    const newWeightNode = new Node(row, column, NodeWeightValue, NodeType.Weight);
    // Cannot set start or target node as weight
    if (
      this.isSameNodePosition(this.startNode, newWeightNode) ||
      this.isSameNodePosition(this.targetNode, newWeightNode)
    ) {
      return;
      // throw new Error(`ERROR: Cannot set startNode or targetNode as a weightNode`);
    }
    // Update our nodes
    this.setNode(newWeightNode);
    // Push into wall's
    this.weights.push(newWeightNode);
    this.updateNodesWeight();
  }

  removeWeight(row: number, column: number): void {
    // Not valid position
    if (!this.isValidPosition(row, column)) {
      return;
    }
    // oldNode is not Wall
    if (!this.isWeight(row, column)) {
      return;
    }

    const indexToRemove = this.weights.findIndex(
      (weight) => weight.rowIndex === row && weight.columnIndex === column
    );
    if (indexToRemove !== -1) {
      // Remove from walls
    ...
    this.updateNode(
      this.getNode(row, column),
      new Node(row, column, 0, NodeType.Normal)
    );
    // Update the weights
    this.updateNodesWeight();
  }
  ...
  getValidNeighbors(node: Node): Node[] {
    ...
    directions.forEach(([directionRow, directionColumn]) => {
      const newRow = node.rowIndex + directionRow;
      const newColumn = node.columnIndex + directionColumn;
  ...
  updateNodesWeight(): void {
    ...
    while (true) {
      ...
      neighbors.forEach((neighbor) => {
        ...
        const newWeight =
          currentNode.weight + additionalWeight;
      ...
      this.nodes.forEach((row) => {
        row.forEach((node) => {
          if (
            !node.isVisited &&
            (nextNode === null || node.weight < nextNode.weight)
          ) {
            nextNode = node;
          }
        });
  ...
  calculateEuclideanDistance(node: Node, target: Node): number {
    const euclideanDistance = Math.sqrt(
      Math.pow(node.rowIndex - target.rowIndex, 2) +
        Math.pow(node.columnIndex - target.columnIndex, 2)
    );
  ...
  isValidPosition(row: number, column: number): boolean {
    return (
      row >= 0 && row < this.numRows && column >= 0 && column < this.numColumns
    );
  }
  ...
  isEdgeNode(node: Node): boolean {
    return (
      node.rowIndex == 0 ||
      node.columnIndex == 0 ||
      node.rowIndex == this.numRows ||
      node.columnIndex == this.numColumns
    );
  }
  ...
  isWeight(row: number, column: number): boolean {
